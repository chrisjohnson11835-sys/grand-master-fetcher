...
class SECClient:
    def __init__(self, ua: str, spacing_seconds: float, max_retries: int, backoff_base: float, jitter_range: Tuple[float,float]):
        self.ua = ua; self.rl = RateLimiter(spacing_seconds)
        self.max_retries = max_retries; self.backoff = backoff_base; self.jitter = jitter_range
    def _req(self, url, params=None):
        ...
        while True:
            ...
            if r.status_code in (429,503):
                ...
                attempt += 1
                if attempt > self.max_retries: raise RuntimeError(f"SEC returned {r.status_code} repeatedly for {url}")
                continue
...
def parse_atom_entries(xml: str) -> List[Dict]:
    soup = BeautifulSoup(xml, "xml"); out = []; import re
    for e in soup.find_all("entry"):
        title=(e.find("title").text if e.find("title") else "").strip()
        updated=(e.find("updated").text if e.find("updated") else "").strip()
        link_el=e.find("link"); link=link_el["href"].strip() if link_el and link_el.has_attr("href") else ""
        summary=(e.find("summary").text if e.find("summary") else "").strip()
        m=re.match(r"^([^-]+)-", title); form=m.group(1).strip() if m else ""
        mcik=re.search(r"\((\d{10})\)", title); cik=mcik.group(1) if mcik else ""
        company=""; m2=re.search(r"-\s*(.*?)\s*\(\d{10}\)", title); company=m2.group(1).strip() if m2 else ""
...
def parse_html_entries(html: str) -> List[Dict]:
    soup=BeautifulSoup(html,"lxml"); table=soup.select_one("table.tableFile2") or soup.select_one("table.tableFile"); out=[]
    if not table: return out
    rows=table.find_all("tr")
    if rows and rows[0].find_all(["th","td"]):
        header_text=rows[0].get_text(" ", strip=True).lower()
        if any(k in header_text for k in ("form","company","file","date")): rows=rows[1:]
    for tr in rows:
        tds=tr.find_all("td"); if len(tds)<4: continue
        form=tds[0].get_text(" ", strip=True); company_col=tds[1].get_text(" ", strip=True)
        link_el=tds[1].find("a", href=True); link="https://www.sec.gov"+link_el["href"] if link_el else ""
        date_time=tds[3].get_text(" ", strip=True); cik=""
        if link_el and "CIK=" in link_el["href"]:
            import urllib.parse as up; qs=up.parse_qs(up.urlparse(link_el["href"]).query)
            if "CIK" in qs and qs["CIK"]: cik=qs["CIK"][0].zfill(10)
