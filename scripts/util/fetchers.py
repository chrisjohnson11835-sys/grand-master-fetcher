...
class SECClient:
    def __init__(self, ua:str, spacing_seconds:float, max_retries:int, backoff_base:float, jitter_range:Tuple[float,float]):
        self.ua=ua; self.rl=RateLimiter(spacing_seconds); self.max_retries=max_retries; self.backoff=backoff_base; self.jitter=jitter_range
...
def parse_html_entries(html:str)->List[Dict]:
    soup=BeautifulSoup(html,"lxml"); table=soup.find("table",{"class":"tableFile2"}); out=[]
    if not table: return out
    rows=table.find_all("tr")[1:]
    for tr in rows:
        tds=tr.find_all("td"); if len(tds)<5: continue
        company_col=tds[1].get_text(" ", strip=True); form=tds[0].get_text(" ", strip=True)
