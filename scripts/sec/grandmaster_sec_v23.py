        ...
        if kind=='fulltext':
            ...
            while pages < max_pages:
                try:
                    ...
                    print(f"[WARN] HTML fetch error (page {pages}, start={start_idx}): {e}")
                    break
                ...
                for e,dt in zip(page_entries,dts):
                    if _within(dt,start_et,end_et):
                        entries.append(e)
                pages+=1
                start_idx+=count_per_page
                if page_max < start_et:
                    break
        elif kind=='atom':
            start_idx=0
            while pages < max_pages:
                try:
                    ...
                    print(f"[WARN] Atom fetch error (page {pages}, start={start_idx}): {e}")
                    break
...
def _daily_index_collect(client, cfg, start_et, end_et):
    ...
    for d in sorted(dates):
        ...
        try:
            txt=client._req(url).text
        except Exception as e:
            print(f"[WARN] Daily index fetch failed for {d}: {e}")
            continue
        ...
        for r in rows:
            ...
            try:
                hdr=client._req(f_url).text[:100000]
            except Exception as e:
                print(f"[WARN] Filing header fetch failed: {f_url} :: {e}")
                continue
            acc=acceptance_from_header_txt(hdr)
            if acc and _within(acc,start_et,end_et):
                out.append({
                    "title":f"{r['form']} - {r['company']}",
                    "form":r["form"],
                    "company":r["company"],
                    "cik":r["cik"],
                    "updated":acc.isoformat(),
                    "link":f_url,
                    "summary":""
                })
    return out
...
def main():
    ...
    try:
        ...
        for e in entries:
            if e.get("form","") not in forms_ok:
                continue
            cik=e.get("cik","")
            if per_cik_cap>0 and cik:
                if by_cik[cik] >= per_cik_cap:
                    continue
                by_cik[cik]+=1
            strict.append(e)
        ...
        if strict:
            ...
            for e in strict:
                try:
                    dts.append(parse_edgar_datetime_et(e.get("updated","")))
                except:
                    pass
            if dts:
        ...
        for e in strict[:top_doc_parse]:
            if time.monotonic()-t0 > soft_budget_sec:
                print("[WARN] Soft time budget reached; skipping further doc parsing.")
                break
            if e.get("form") in ("8-K","4"):
                excerpt=minimal_doc(e)
                if excerpt:
                    if e["form"]=="8-K":
                        e["eightk_items"]=extract_eightk_items(excerpt)
                    if e["form"]=="4":
                        e["form4_codes"]=extract_form4_codes(excerpt)
        ...
        if h_url and secret and h_url.startswith("http"):
            for p in (raw_path, snap_json_path, snap_csv_path, stats_path):
                try:
                    post_file(h_url, secret, p, "/public_html/data")
                except Exception as ue:
                    print("[UPLOAD WARN]", ue)
        ...
        if os.environ.get("CI","")=="true" and not passed:
            sys.exit(1)
    except Exception as ex:
